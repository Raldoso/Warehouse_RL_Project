...
class WarehouseEnv(gym.Env):
    ...
    def __init__(self, numStores, maxage, episodeLength):
        self.daycount = 0
        self.stores = []
        self.max_age = maxage
        self.episode_length = episodeLength
        ...
        self.observation_space = spaces.Dict(
            {
                ...
                "store_info":   spaces.Box(low=np.array([0, 0]), high=np.array([200, 100]), dtype=int),

                # TODO calculate store info max
 
                #warehouse info
                #conatins 2 values, sum of storage, last ordered amount

                "warehouse_info": spaces.Box(low=0, high=self.maxorder, dtype=int, shape=(self.max_age+4), dtype=int)


            })
        #maximum amount to order
        self.action_space = spaces.Discrete(self.maxorder)
    ...
    def step(self, action):
        ...
        return observation, reward, self.daycount == self.episode_length, None
    ...
    class Store():
        def __init__(self, avg_range, std_range, max_age):
        ...
        def get_sold_amount(self):
            ...
            self.min_items = round(self.avg * 0.2)
